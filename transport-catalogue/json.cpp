...
void Node::Print(ostream& output) const {
	if (IsNull()) {
		...
		string bool_str = AsBool() ? "true"s : "false"s;
		output << bool_str;
	}
	else if (IsString()) {
		output << '\"';
		...
		if (!AsArray().empty()) {
			for (auto it = AsArray().begin(); next(it) != AsArray().end(); ++it) {
				it->Print(output);
				output << ',';
			}
			AsArray().back().Print(output);
		}
		...
		if (!AsDict().empty()) {
			for (auto it = AsDict().begin(); next(it) != AsDict().end(); ++it) {
				output << '"' << it->first << '"' << ':';
				it->second.Print(output);
				output << ',';
			}
			auto it = AsDict().rbegin();
			output << '"' << it->first << '"' << ':';
			it->second.Print(output);
		}
		output << '}';
